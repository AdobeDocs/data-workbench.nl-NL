...
<table id="table_E28A6D31E7D941F7A0C2048F0F0F7838"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> Deel rijke klantensegmenten die in Data Workbench aan Adobe Experience Cloud worden gecreeerd. Dankzij de integratie met profielen en soorten publiek kunnen de Adobe Experience Cloud </a> en andere Adobe-analysemogelijkheden de mogelijkheid bieden om rijke publiekssegmenten die in de Data Workbench zijn gemaakt, te benutten. <a href="https://www.adobe.com/solutions/digital-marketing.html?promoid=KLXLZ" format="https" scope="external"> Dit vereist dat u de Experience Cloud gevormd en lopend hebt. </a><p>Dankzij de integratie met de Master Experience Cloud kunt u voorspellende functies uitvoeren, zoals clustering of het scoren van eigenschappen, en segmenten vervolgens uitduwen naar de grotere Adobe Experience Cloud of andere producten, zoals Adobe Target en Adobe Experience Manager. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> De visualisatie van het Koord verstrekt een andere mening van <a href="https://experienceleague.adobe.com/docs/data-workbench/using/client/analysis-visualizations/correlation-analysis/c-correlation-analysis.html" format="http" scope="external"> de Matrijs van de Correlatie </a>. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td colname="col1"> <b></b> Verspreiding </td> 
   <td colname="col2"> In het paneel van Vinders, kunt u op <span class="uicontrol"> Dimension nu met de rechtermuisknop klikken </span> tabel en <span class="uicontrol"> Type van Dimension &gt; Tijd </span> selecteren. In de zoekresultaten wordt een lijst met tijdafmetingen weergegeven. <p> <img placement="break" id="image_BA6A577501A74088B55A881554C89F10" src="assets/time_dim.png" /> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <b></b> Lockfeature </td> 
   <td colname="col2"> Met de nieuwe functie <b>Vergrendelen</b> wordt een pictogram weergegeven op de werkbalk wanneer een werkruimte is vergrendeld. U kunt de werkruimte ontgrendelen door op het menu <b>Toevoegen</b> te klikken en vervolgens op <b>Tijdelijk ontgrendelen</b> te klikken. </td> 
  </tr> 
  <tr> 
   <td colname="col1"> Logische operatoren en nieuwe metrische functies in het <b>deelvenster Filter </b> </td> 
   <td colname="col2"> EN/OF de logische exploitanten werden toegevoegd aan <b>het Comité van de Filter</b>, die u toestaan om metriek te verbinden of toe te voegen wanneer het filtreren van gegevens. Als u de maateenheden wijzigt, wordt het Filterpercentage dienovereenkomstig aangepast en weergegeven. </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> Microsoft Windows 8.1 64-bits wordt <a href="https://experienceleague.adobe.com/docs/data-workbench/using/install/c-data-workbench-client-install.html" format="https" scope="external"> nu ondersteund </a> voor clientinstallatie. </td> 
  </tr> 
 </tbody> 
</table>
...
* **Update voor snelle verbeteringen** in de samenvoeging. Voeg parameters toe of verander waarden aan de volgende configuratiedossiers om uit snelheidsverbeteringen in Data Workbench tijdens een transformatie voordeel te halen.
   ...
   * **Log Processing Mode.cfg** (  [!DNL E:\Server\Profiles\<your profile>\Dataset\Log Processing Mode.cfg])
      ...
      >
      >  **Aanbevolen wordt om de instelling Interval opslaan te verlagen tot 1800 seconden (30 minuten)**. Het standaard interval voor opslaan is 3600 seconden (1 uur). (Als deze regel moet worden toegevoegd aan [!DNL Log Processing Mode.cfg], ziet u dat er twee (2) voorloopruimten zijn).
      >
      >  
      ```
      ...
      >  ```
      >
      >  * Het wordt geadviseerd dat de nieuwe parameters van de Wijze van de Verwerking van het Logboek niet op een dataset worden toegepast die met **CrossRows** gebruikend de **al** verrichting wordt gevormd. Met deze instellingen kan het geheugengebruik voor deze bewerking te groot worden voor de omgeving.


* **Adobe Target met DWB-integratieupdate**. Een nieuw exportbestand, [!DNL ExportIntegration.exe], vervangt het bestaande [!DNL TnTSend.exe]-bestand op de Insight Server ( [!DNL E:\Server\Scripts\TnTSend.exe]). Dit nieuwe exportbestand ondersteunt zowel Adobe Target-integratie als coördinatie met het nieuwe Master marketingprofiel (MMP) en Adobe Audience Manager.
...
* **Werk de client bij vanaf de server**.
   ...
   Nadat de server is bijgewerkt, kan de client automatisch worden bijgewerkt als het [!DNL Insight.cfg]-bestand op de juiste wijze is geconfigureerd:
   ...
   1. [!DNL Insight.cfg] bewerken

      * [!DNL Proxy Password = string:] wijzigen
...
* **Aanbeveling: Lettertypen voor Chinese en Japanse versies**:
   ...
   * SimSun
   ...
   >[!NOTE]
   >
   >** SimSuncan wordt gebruikt voor Chinees en Japans. Als u probeert in tekens van halve bytes te schrijven in het Japans, moet u ook *MS Mincho* opnemen. U kunt deze parameters toevoegen om deze lettertypen in [!DNL Insight.cfg] in te schakelen.
...
Als u **Best Fit Attribution** in Data Workbench wilt uitvoeren, moet u nieuwe certificaten ontvangen van Adobe ClientCare voor uw client, server en rapportserver (.pem-bestanden) ter ondersteuning van Adobe Analytics Premium. Elk van de nieuwe certificaten heeft deze parameter:

```
Product = Premium
```

Het Premium-pakket kan worden gedownload op **[!UICONTROL Software and Docs]** onder het tabblad **[!UICONTROL Getting Started]** in de werkruimte **Profielen en Opzoeken** bestanden. Ga naar `Profiles - Current\DataWorkBench\<language>\Data WorkBench_6.30-en-us\Premium_6.30_en-us.zip`.  Nadat het **Premium**-profiel op uw server is geladen, moet u een *Premium*-parameter toevoegen aan uw aangepaste [!DNL Profile.cfg]-bestand. Zo kunt u in uw aangepaste profiel de menu&#39;s, visualisaties en werkruimten opnemen als onderdeel van Adobe Analytics Premium.

## Vaste bugs {#section-e1e3df3da3594987a325344cd5e8b855}

* Probleem verholpen waarbij de grootste elementen ontbraken in de **Dichtheid-kaart**-visualisatie.
* Correctie van het probleem in **Dichtheid Kaart** waar het gebied van elementen niet het aandeel van de metrische waarde portretteerde.
* Tijdens het slepen van metrische gegevens van **[!UICONTROL Finders]** naar de metrische legenda buiten de metrische kolom werd de legenda uit de werkruimte verwijderd. Dit probleem is nu opgelost.
* Bij **[!UICONTROL Print Workspace]** met de opties **[!UICONTROL Sidebar]** en **[!UICONTROL Both]** zijn de copyrightgegevens niet op de afgedrukte pagina opgenomen. Dit probleem is nu opgelost.

## Bekende problemen {#section-751e8698e9f14b75a85f63253782be41}

* Gebruikers van **AMD Radeon™ grafische kaarten** dienen een update uit te voeren naar de nieuwste grafische stuurprogramma&#39;s. Sommige eerdere versies van het stuurprogramma ondersteunen OpenGL 3.2, maar zijn inconsistent.
* Output die door **de configuratie van de Uitvoer van het Segment** zonder een kopbalverklaring wordt geproduceerd kan in een valse kopbal resulteren die aan het begin van het dossier verschijnt dat met de eerste reeks rijen in conflict brengt.
* **Voeg** Dimensionsis toe met alleen de  *Extended-Dimension*. Als tussenoplossing kunt u het gereedschap **Finders** gebruiken om afmetingen naar tabellen te slepen.
